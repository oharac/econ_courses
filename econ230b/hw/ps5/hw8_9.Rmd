...
In equilibrium, we know the costs of each driver must be the same. So $45w=(20+\frac{N_1}{100+H})w$. Then $$N_1=2500+25H.$$ Each person's value of time saved is $w(45-20+\frac{N_1}{100+H})$. The maximization problem is:

 $$\max\limits_{H} \hspace{5mm} N_1w(45-20-\frac{N_1}{100+H})-pH \hspace{2mm} s.t. \hspace{2mm} N_1=2500+25H$$
$$\Leftrightarrow$$
$$\max\limits_{H} \hspace{5mm} -pH$$

Since this expression is strictly decreasing in H, the value of H that minimizes the expression is 0 unless H is so large that $$45>20+\frac{6000}{100+H}$$

This implies $H>140$. The new maximization problem becomes:

$$\max\limits_{H} \hspace{5mm} 6000w(25-\frac{6000}{100+H})-pH \hspace{2mm} $$

FOC: $\frac{6000}{100+H}=\sqrt{\frac{c}{w}}$.

Then $H^*=600\sqrt{\frac{c}{w}}-100$.

The corresponding number of commuters is 6000 and the time it takes is $20+10\frac{w}{c}$.
...
Recall the toll is 12.5w and the government will charge enough to cover the cost of expansion. The maximization problem becomes:

$$\max\limits_{H} \hspace{5mm} N_1w(25-\frac{N_1}{100+H})-pH \hspace{2mm} s.t. \hspace{2mm} N_1=\frac{1}{w}(25w-12.5w)(100+H)$$

Then $H^*= \frac{6000}{\sqrt{12.5}}-100$.
...
* Let $t_1$ be the departure time of the first commuter, $t_N$ of the last, $\beta$ the cost of arriving early and $\gamma$ the cost of arriving late. Using the equal cost condition between first and last commuters: 

\begin{align*}
  &\beta (t^* - t_1) = \gamma (t_N - t^*) \\
  &\beta (t^* - t_1) = \gamma (\frac{N}{s} + t_1 - t^*) 
      &\text{(since $t_N - t_1 = \frac{N}{s}$)}\\
  &(\beta + \gamma)t^* - (\beta + \gamma)t_1 = \frac{\gamma N}{s} \\
  &t^* - t_1 = \frac{\gamma N}{(\beta + \gamma)s} \\
  &\boxed{t_1^* = t^* - \frac{\gamma}{(\beta + \gamma)}\frac{N}{s}} \\
  \Rightarrow &\boxed{t_N^* = t^* + \frac{\beta}{(\beta + \gamma)}\frac{N}{s}}
\end{align*}


* For the early and late time, we can see that when $\gamma$ increases: 

\begin{align*}
    &\frac{\dd t_1^*}{\dd \gamma} < 0  \\
    &\frac{\dd t_N^*}{\dd \gamma} < 0  
\end{align*}

Hence, as the cost of arriving late increases, but the earliest and latest departure time reduce. 

* For the commuter that arrives just in time: 

\begin{align*}
   &\boxed{\tilde{t}^* = t^* - \frac{\beta}{\alpha}\frac{\beta}{(\beta + \gamma)}\frac{N}{s}} \\
   &\frac{\dd \tilde{t}^*}{\dd \gamma} < 0
\end{align*}

His departure time also reduces. 
